...
class TestCreateCourier:
    ...
    def test_successful_create_courier(self, delete_user):
        response = ScooterApi.create_courier(TestDataHelper.generate_registration_body(TestDataHelper.generate_random_login()))
    ...
    def test_create_duplicate_courier_fails(self, delete_user):
        user_body = TestDataHelper.generate_registration_body(TestDataHelper.generate_random_login())
    ...
    def test_create_courier_missing_field_login_fails(self, missing_field):
        user_body = TestDataHelper.generate_registration_body(TestDataHelper.generate_random_login())
    ...
    @allure.title("Тест создания курьера с основными необходимыми полями")
    def test_successful_courier_creation_requires_all_mandatory_fields(self, delete_user):
        random_login = TestDataHelper.generate_random_login()
        user_body = TestDataHelper.generate_registration_body(random_login)
        incomplete_user_body = {
            "password": user_body["password"],
            "firstName": user_body["firstName"]
        }
        response = ScooterApi.create_courier(incomplete_user_body)
        assert response.status_code == 400
        response_body = response.json()
        assert response_body['message'] == 'Недостаточно данных для создания учетной записи'
        user_body = TestDataHelper.generate_registration_body(random_login)
        user_body.pop("firstName")
        response = ScooterApi.create_courier(user_body)
